...
module.exports = {
    ...
    filters: {
        cite: function (key) {
            console.log('calling cite', key);
            // var bibData = this.config.get('bib-data')
            var idx = _.findIndex(
                    BIB_DATA,
                    function (cite) { return cite.citationKey === key.toUpperCase() }
                ),
                refsPath = this.config.get('pluginsConfig.bibtex.references-url', null);
                linkText = '<a href="$URL#ref-item-$NUMBER">$NUMBER</a>';

            if (idx < 0 ) { return "[Citation not found]"; }

            var citation = BIB_DATA[idx];
            if (!citation.used) {
                console.log('citation not used');
                citation.used = true;
                CITATION_USES += 1;
                var bibCount = CITATION_USES;
                console.log('bibCount? ', bibCount);
                // this.config.set('bibCount', bibCount);
                // console.log('new bib count?', this.config.get('bibCount'));
                citation.number = bibCount;
                BIB_DATA[idx] = citation;
                // this.config.set('bib-data', bibData);
            }
            if (refsPath) {
    ...
    hooks: {
        init: function () {
            console.log( 'bibtex hook');
            var bibTexPath = this.config.get('pluginsConfig.bibtex.bibtex-path', 'literature.bib');
            console.log('BitTex Path', bibTexPath);
            var bib = fs.readFileSync(bibTexPath, 'utf8');
            var jsonBib = bibtexParse.toJSON(bib)
            console.log(jsonBib);
            BIB_DATA = jsonBib;
            // this.config.set('bib-data', jsonBib);
    ...
    blocks: {
        references: {
            process: function (blk) {
                ...
                // usedBib = _.filter(this.config.get('bib-data'), 'used');
                usedBib = _.filter(BIB_DATA, 'used');
                console.log('USED BIBS', usedBib);
